    ...
    var wwctrialsMain = {
        ...
        parallax: function() {
            ...
            if (wwctrialsMain.isMobile.any()) {
                $('.hero-image-webpart').each(function (i, e) {
                    $(this).find('.img-bg').css("background-position", "50% "  + bgImageTop + "px");
            ...
            var getImageSize = function() {
                var img = new Image;
                img.src = $('.hero-image-webpart .img-bg').data('imgurl');
                bgImgWidth = img.width;
                bgImgHeight = img.height;
                imgCoef =bgImgWidth / bgImgHeight;
            }
            getImageSize();

            var calculateCoverOffset = function() {
                var containerW = $('.hero-image-webpart .img-bg').innerWidth();
                var containerH = $('.hero-image-webpart .img-bg').innerHeight();
                var calImageWidth = containerW;
                var calImageHeight = containerW / imgCoef;
                if (calImageHeight < containerH) {
                    var calImageHeight = containerH;
                    var calImageWidth = containerH  * imgCoef;
                }
                bgImageTop = Math.round((containerH - calImageHeight) / 2);
                var objTop = $('.hero-image-webpart .img-bg').offset().top;
                var scrTop = $(window).scrollTop();
                if (objTop-scrTop + bgImageTop < 0) {
                    $('.hero-image-webpart .img-bg').css("background-position", "50% " + (bgImageTop-((bgImageTop+objTop-scrTop)/2))  + "px");
                } else {
                    $('.hero-image-webpart .img-bg').css("background-position", "50% " + bgImageTop  + "px");
                }
            }
        ...
        videoModal: function () {
            ...
            $modal.on('show.bs.modal', function(event) {
				...
				if ($(event.relatedTarget).parent().hasClass('featured-resource') && libSignup != '1') {
					console.log('video popup 2');
					return event.preventDefault();
				}
					console.log('video popup 3');
        ...
        librarySignupModal: function () {
			//$(document).off('.featured-resource > a');
            ...
            if ($modal.length > 0) {
               ...
               // var $featuredResources = $('.featured-resource');
                //$featuredResources.on('click', 'a', function (event) {
                $(document).on('click', '.library-featured a, .featured-resource a', function (event) {
                    var $link = $(this);
        ...
        librarySignupModal: function () {
            ...
            var $signupSuccess = $modal.find('.feature-signup-success a');
            if ($modal.length > 0) {
                var $gatedResources = $('a.gated');
                $gatedResources.on('click', function (event) {
                    var $link = $(this);
					//console.log($link);
        ...
        imageSliderHero: function () {
            $('.image-slider-hero').each(function() {
            console.log('trying to initialize slider 2');
                var $slider = $(this);
                $slider.slick({
                    arrows: true,
                    dots: true,
